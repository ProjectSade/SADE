...
void QTRSensors::readCalibrated(unsigned int *sensor_values, unsigned char readMode)
{
    ...
    for(i=0;i<_numSensors;i++)
    {
        ...
        signed int x = 0;
        if(denominator != 0)													//aca no habia algo							
            x = ((((signed long)sensor_values[i]) - calmin) * 1000 / denominator);    ///aca cambiar flacos de comparacion
        if(x < 0)
            x = 0;
        else if(x > 1000)
            x = 1000;		//para negro estaba en +200
        sensor_values[i] = x+200;  ///////////////////////INVERTI ACA  con 1000 PARA SENSADO EN NEGRO***************
    }								///************************************
									///************************************
    ...
    unsigned char readMode, unsigned char white_line)
{
    ...
    for(i=0;i<_numSensors;i++) {
        ...
        if(value > 200) {///estaba en 200 por defecto********************
            on_line = 1;
        }

        // only average in values that are above a noise threshold
        if(value > 50) {  ///por defecto estaba en 50*******************
