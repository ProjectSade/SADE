...
void QTRSensors::readCalibrated(unsigned int *sensor_values, unsigned char readMode,int flanco_sens)
{
    int i;
    ...
    for(i=0;i<_numSensors;i++)
    {
        ...
        if(x < 0)
            ...
            x = 1000;
        sensor_values[i] = x + flanco_sens;  /// CAMBIO DEL FLANCO DE SENSADO 
    }
...
int QTRSensors::readLine(unsigned int *sensor_values,
    unsigned char readMode, unsigned char white_line,int flanco_sens, int keep_track, int noise_threshold )
{
    ...
    readCalibrated(sensor_values, readMode,flanco_sens);
    ...
    for(i=0;i<_numSensors;i++) {
        ...
        if(white_line)
            value = 1000-value; ///********<<<<<tenia 1000

        // keep track of whether we see the line at all
        if(value > keep_track) { ///estaba en 200
            on_line = 1;
        }
